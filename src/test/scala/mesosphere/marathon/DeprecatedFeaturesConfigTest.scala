...
class DeprecatedFeaturesConfigTest extends UnitTest {
  ...
  "isValid()" should {
    "returns true when only softRemoved features are enabled" in {
      val deprecatedFeatureConfig = DeprecatedFeatureConfig(
        currentVersion = currentVersion,
        deprecatedFeatureSettings = Map(hardRemovedIn170 -> true))
    ...
    "returns true when hardRemoved features are disabled" in {
      val deprecatedFeatureConfig = DeprecatedFeatureConfig(
        currentVersion = SemVer(1, 7, 0),
        deprecatedFeatureSettings = Map(hardRemovedIn170 -> false))
  ...
  "isEnabled(...)" should {
    ...
    "returns true for features not yet soft-removed" in {
      val deprecatedFeatureConfig = DeprecatedFeatureConfig(
        currentVersion = currentVersion, Map.empty)
    ...
    "returns false for features not yet soft-removed but explicitly disabled" in {
      val deprecatedFeatureConfig = DeprecatedFeatureConfig(
        currentVersion = currentVersion,
        deprecatedFeatureSettings = Map(softRemovedIn170 -> false))
