...
class VolumeTest extends UnitTest {
  ...
  trait Fixture {
    val rootVolNoConstraints = PersistentVolumeInfo(size = 1024, constraints = Set.empty)
    val pathVolWithConstraint =
    ...
    val secretVol = VolumeWithMount(volume = SecretVolume(None, secret = "secret-name"), mount = VolumeMount(None, "secret-path", true))
  }
  ...
  "Volume" should {
    ...
    "validating PersistentVolumeInfo constraints rejects unsupported fields" in {
      val pvi = PersistentVolumeInfo(1024, `type` = DiskType.Path, constraints = Set(constraint("invalid", "LIKE", Some("regex"))))
    ...
    "validating PersistentVolumeInfo constraints rejected for root resources" in {
      val result =
        validate(PersistentVolumeInfo(1024, `type` = DiskType.Root, constraints = Set(constraint("path", "LIKE", Some("regex")))))
      result.isSuccess shouldBe false
    ...
    "validating PersistentVolumeInfo constraints rejects bad regex" in {
      val pvi = PersistentVolumeInfo(1024, `type` = DiskType.Path, constraints = Set(constraint("path", "LIKE", Some("(bad regex"))))
      val result = validate(pvi)
      result.isSuccess shouldBe false
    ...
    "validating PersistentVolumeInfo rejects an empty profileName" in new Fixture {
      val result = validate(PersistentVolumeInfo(size = 1024, `type` = DiskType.Mount, profileName = Some("")))
      result.isSuccess shouldBe false
    ...
    "validating PersistentVolumeInfo maxSize parameter wrt type" in new Fixture {
      val resultRoot = validate(PersistentVolumeInfo(1024, `type` = DiskType.Root, maxSize = Some(2048)))
      ...
      val resultPath = validate(PersistentVolumeInfo(1024, `type` = DiskType.Path, maxSize = Some(2048)))
    ...
    "validating that DiskSource asMesos converts to an Option Mesos Protobuffer" in {
      ...
      val Some(pathCsiDisk) = DiskSource
        .fromParams(
          DiskType.Path,
          Some("/path/to/folder"),
          Some("csiPathDisk"),
          Some(Map("pathKey" -> "pathValue")),
          Some("pathProfile"),
          None
        )
        .asMesos
      ...
      val Some(mountCsiDisk) = DiskSource
        .fromParams(
          DiskType.Mount,
          Some("/path/to/mount"),
          Some("csiMountDisk"),
          Some(Map("mountKey" -> "mountValue")),
          Some("mountProfile"),
          None
        )
        .asMesos
