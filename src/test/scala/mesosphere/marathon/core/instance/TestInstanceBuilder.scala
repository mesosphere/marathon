...
case class TestInstanceBuilder(instance: Instance, now: Timestamp = Timestamp.now()) {
  ...
  def addTaskUnreachable(
      ...
      unreachableStrategy: UnreachableStrategy = UnreachableEnabled()
  ): TestInstanceBuilder = {
...
object TestInstanceBuilder {
  ...
  def emptyInstance(
    ...
    val runSpec =
      AppDefinition(instanceId.runSpecId, unreachableStrategy = unreachableStrategy, versionInfo = VersionInfo.OnlyVersion(version))
    Instance(
  ...
  def newBuilder(runSpecId: PathId, now: Timestamp = Timestamp.now(), version: Timestamp = Timestamp.zero): TestInstanceBuilder =
