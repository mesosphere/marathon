...
class InstanceUpdaterTest extends UnitTest with Inside {
  ...
  "A terminal instance with goal stopped should not be expunged" in {
    ...
    val app = new AppDefinition(PathId("/test"), role = Some("someRole"))
  ...
  "suspend reservation when resident instance is terminal" in {
    ...
    val app = AppDefinition(PathId("/test"), role = Some("someRole"))
  ...
  class Fixture {
    ...
    val instance = Instance(
      instanceId, Some(agentInfo), instanceState, Map(taskId -> task), app, None, Instance.defaultMesosRole)
  }
}
