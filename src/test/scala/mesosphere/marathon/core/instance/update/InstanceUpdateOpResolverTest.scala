...
import mesosphere.marathon.state.{AppDefinition, PathId, Timestamp, UnreachableDisabled, UnreachableStrategy, VersionInfo}
...
class InstanceUpdateOpResolverTest extends UnitTest with Inside {
  ...
  class Fixture(unreachableStrategy: UnreachableStrategy = UnreachableDisabled) {
    ...
    lazy val reservedInstance = TestInstanceBuilder
      .scheduledWithReservation(AppDefinition(appId))
