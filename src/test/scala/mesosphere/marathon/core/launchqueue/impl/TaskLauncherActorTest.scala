  ...
  "TaskLauncherActor" should {
    ...
    "match offer when in progress and offer received" in new Fixture {
      ...
      Mockito
        ...
        .thenReturn(InstanceTracker.InstancesBySpec.forInstances(f.runningInstance, f.scheduledInstance))
      val offer = MarathonTestHelper.makeBasicOffer().build()
    ...
    "update the internal when instance update is received" in new Fixture {
      ...
      Mockito
        .when(instanceTracker.instancesBySpecSync)
        .thenReturn(InstanceTracker.InstancesBySpec.forInstances(f.runningInstance, provisionedInstance))
      launcherRef ! update
    ...
    "not use unreachable instance when matching an offer if unreachable but not inactive" in new Fixture {
      ...
      Mockito
        .when(instanceTracker.instancesBySpecSync)
        .thenReturn(InstanceTracker.InstancesBySpec.forInstances(lostInstance, Instance.scheduled(f.app)))
