...
import mesosphere.marathon.state.{AppDefinition, Instance => StateInstance, PathId}
  ...
  forAll (metricsModules) { (name: String, metricsModule: MetricsModule) =>
    s"InstanceTrackerActor (metrics = $name)" should {
      ...
      "updates repository as well as internal state for instance update" in {
        ...
        verify(f.repository).store(StateInstance.fromCoreInstance(helper.wrapped.instance))
      ...
      "fails when repository call fails for update" in {
        ...
        f.repository.store(StateInstance.fromCoreInstance(instance)) returns Future.failed(new RuntimeException("fail"))
