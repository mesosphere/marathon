...
/**
  ...
  */
class TaskKillServiceMock(system: ActorSystem) extends TaskKillService with InstanceConversions {
  ...
  private[this] def killTaskById(taskId: Instance.Id, reason: TaskKillReason): Future[Done] = {
    ...
    val update = customStatusUpdates.getOrElse(taskId, MesosStatusUpdateEvent("", Task.Id(taskId.idString), "TASK_KILLED", "", appId, "", None, Nil, "no-version"))
  ...
  override def killUnknownTask(taskId: Task.Id, reason: TaskKillReason): Future[Done] = killTaskById(taskId, reason)
}
