...
class TaskKillServiceMock(system: ActorSystem) extends TaskKillService {
  ...
  private[this] def killTaskById(taskId: Task.Id, reason: TaskKillReason): Future[Unit] = {
  ...
  override def killTask(task: Task, reason: TaskKillReason): Future[Unit] = killTaskById(task.taskId, reason)

  override def killUnknownTask(taskId: Id, reason: TaskKillReason): Future[Unit] = killTaskById(taskId, reason)
}
