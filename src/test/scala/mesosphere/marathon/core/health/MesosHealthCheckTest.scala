...
class MesosHealthCheckTest extends UnitTest {
  ...
  "MesosHealthCheck" should {
    ...
    "Write COMMAND health check" in {
      ...
      """
      JsonTestHelper
        .assertThatJsonOf(MesosCommandHealthCheck(command = Command("echo healthy")))
    ...
    "Write Mesos HTTP health check" in {
      ...
      """
      JsonTestHelper
        .assertThatJsonOf(mesosHttpHealthCheckWithPortIndex)
      ...
      """
      JsonTestHelper
        .assertThatJsonOf(mesosHttpHealthCheckWithPort)
    ...
    "Write Mesos HTTPS health check" in {
      ...
      """
      JsonTestHelper
        .assertThatJsonOf(mesosHttpHealthCheckWithPortIndex.copy(protocol = Protocol.MESOS_HTTPS))
        .correspondsToJsonString(portIndexJson)
      ...
      """
      JsonTestHelper
        .assertThatJsonOf(mesosHttpHealthCheckWithPort.copy(protocol = Protocol.MESOS_HTTPS))
        .correspondsToJsonString(portJson)
    }
    ...
    "Write Mesos TCP health check" in {
      ...
      """
      JsonTestHelper
        .assertThatJsonOf(mesosTcpHealthCheckWithPortIndex)
        .correspondsToJsonString(portIndexJson)
      ...
      """
      JsonTestHelper
        .assertThatJsonOf(mesosTcpHealthCheckWithPort)
        .correspondsToJsonString(portJson)
    }
  }
  ...
  def buildIfMatches(app: AppDefinition): Option[(MesosProtos.TaskInfo, NetworkInfo)] = {
    ...
    val resourceMatch = RunSpecOfferMatcher.matchOffer(app, offer, Seq.empty, config.defaultAcceptedResourceRolesSet, config, Seq.empty)
    resourceMatch match {
