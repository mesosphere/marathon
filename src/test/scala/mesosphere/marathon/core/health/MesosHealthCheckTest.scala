...
class MesosHealthCheckTest extends MarathonSpec {
  ...
  test("Read Mesos HTTP health check (portIndex may be provided for backwards-compatibility)") {
    val json =
      """
        {
          "protocol": "COMMAND",
  ...
  test("MergeFromProto Mesos HTTP HealthCheck with portIndex") {
  ...
  test("MergeFromProto Mesos HTTPS HealthCheck with port") {
    val proto = Protos.HealthCheckDefinition.newBuilder
      .setPath("/health")
  ...
  test("MergeFromProto Mesos HTTP HealthCheck with neither port nor portIndex") {
    val proto = Protos.HealthCheckDefinition.newBuilder
      .setPath("/health")
      .setProtocol(Protocol.MESOS_HTTP)
      .setGracePeriodSeconds(10)
  ...
  test("MergeFromProto Mesos HTTPS HealthCheck") {
    val proto = Protos.HealthCheckDefinition.newBuilder
      .setPath("/health")
      .setProtocol(Protocol.MESOS_HTTPS)
      .setPortIndex(0)
  ...
  test("MergeFromProto Mesos TCP HealthCheck with portIndex") {
    val proto = Protos.HealthCheckDefinition.newBuilder
