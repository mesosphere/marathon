...
import mesosphere.marathon.core.task.Task.{ Reserved, ReservedTask }
...
class TaskStartActorTest extends AkkaUnitTest with Eventually {
  ...
  "relaunch when resident task gets terminal" in {
    ...
    eventually { verify(f.launchQueue, times(1)).addAsync(eq(app), any) }

    // let existing task die
    var instance = TestInstanceBuilder.newBuilder(app.id).addTaskReserved(None).getInstance()
    val reservedTask = instance.appTask.asInstanceOf[Reserved]
    ...
    eventually { verify(f.launchQueue, times(2)).addAsync(eq(app), any) }
  }
