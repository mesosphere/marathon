...
class AssignDynamicServiceLogicTest extends AkkaUnitTest with GroupCreation {
  "applications with port definitions" when {
    ...
    "multiple apps are assigned dynamic app ports" should {
      val app1 = AppDefinition("/app1".toPath, role = "*", portDefinitions = PortDefinitions(0, 0, 0), cmd = Some("sleep"))
      val app2 = AppDefinition("/app2".toPath, role = "*", portDefinitions = PortDefinitions(1, 2, 3), cmd = Some("sleep"))
      val app3 = AppDefinition("/app3".toPath, role = "*", portDefinitions = PortDefinitions(0, 2, 0), cmd = Some("sleep"))
    ...
    "should reassign dynamic service ports specified in the container" in {
      val app = AppDefinition("/app1".toPath, role = "*", portDefinitions = PortDefinitions(10, 11), cmd = Some("sleep"))
      val updatedApp = AppDefinition("/app1".toPath, role = "*", portDefinitions = PortDefinitions(10, 0, 11), cmd = Some("sleep"))
    ...
    "Already taken ports will not be used" in {
      val servicePortsRange = 10.to(20)
      val app1 = AppDefinition("/app1".toPath, role = "*", portDefinitions = PortDefinitions(0, 0, 0), cmd = Some("sleep"))
      val app2 = AppDefinition("/app2".toPath, role = "*", portDefinitions = PortDefinitions(0, 2, 0), cmd = Some("sleep"))
      val rootGroup = createRootGroup(Map(
        app1.id -> app1,
        app2.id -> app2
      ))
    ...
    "Don't assign duplicated service ports" in {
      val app1 = AppDefinition("/app1".toPath, role = "*", portDefinitions = PortDefinitions(0, 10), cmd = Some("sleep"))
    ...
    "Assign unique service ports also when adding a dynamic service port to an app" in {
      val app1 = AppDefinition("/app1".toPath, role = "*", portDefinitions = PortDefinitions(10, 11), cmd = Some("sleep"))
      val originalGroup = createRootGroup(Map(app1.id -> app1))

      val updatedApp1 = AppDefinition("/app1".toPath, role = "*", portDefinitions = PortDefinitions(0, 0, 0), cmd = Some("sleep"))
    ...
    "If there are not enough ports, a PortExhausted exception is thrown" in {
      val app1 = AppDefinition("/app1".toPath, role = "*", portDefinitions = PortDefinitions(0, 0, 0), cmd = Some("sleep"))
      val app2 = AppDefinition("/app2".toPath, role = "*", portDefinitions = PortDefinitions(0, 0, 0), cmd = Some("sleep"))
      val rootGroup = createRootGroup(Map(
        app1.id -> app1,
        app2.id -> app2
      ))
  ...
  def withContainerNetworking(containerization: String, prototype: => Container): Unit = {
    ...
    s"withContainerNetworking using $containerization" should {
      "Assign dynamic service ports specified in the container" in {
        ...
        val app = AppDefinition("/app1".toPath, role = "*", portDefinitions = Seq(), container = Some(container), networks = virtualNetwork, cmd = Some("sleep"))
        val rootGroup = createRootGroup(Map(app.id -> app))
      ...
      "Assign dynamic service ports specified in multiple containers" in {
        ...
        val virtualNetwork = Seq(ContainerNetwork(name = "whatever"))
        val app1 = AppDefinition("/app1".toPath, role = "*", portDefinitions = Seq(), container = c1, networks = virtualNetwork, cmd = Some("sleep"))
        val app2 = AppDefinition("/app2".toPath, role = "*", portDefinitions = Seq(), container = c2, networks = virtualNetwork, cmd = Some("sleep"))
        val rootGroup = createRootGroup(Map(
          app1.id -> app1,
          app2.id -> app2
        ))
      ...
      "Assign dynamic service ports w/ both BRIDGE and USER containers" in {
        ...
        val bridgeModeApp = AppDefinition("/bridgemodeapp".toPath, role = "*", container = bridgeModeContainer, networks = Seq(BridgeNetwork()), cmd = Some("sleep"))
        val userModeApp = AppDefinition("/usermodeapp".toPath, role = "*", container = userModeContainer, networks = Seq(ContainerNetwork("whatever")), cmd = Some("sleep"))
      ...
      "Assign a service port for an app using Docker USER networking with a default port mapping" in {
        ...
        val app1 = AppDefinition("/app1".toPath, role = "*", portDefinitions = Seq(), container = c1, networks = Seq(ContainerNetwork("whatever")), cmd = Some("sleep"))
        val rootGroup = createRootGroup(Map(app1.id -> app1))
        val update = AssignDynamicServiceLogic.assignDynamicServicePorts(servicePortsRange, createRootGroup(), rootGroup)
        update.transitiveApps.filter(_.hasDynamicServicePorts) should be (empty)
      ...
      "Export non-dynamic service ports specified in the container to the ports field" in {
        ...
        val app1 = AppDefinition("/app1".toPath, role = "*", container = Some(container), networks = Seq(ContainerNetwork("foo")), cmd = Some("sleep"))
        val rootGroup = createRootGroup(Map(app1.id -> app1))
        val update = AssignDynamicServiceLogic.assignDynamicServicePorts(servicePortsRange, createRootGroup(), rootGroup)
        update.transitiveApps.filter(_.hasDynamicServicePorts) should be (empty)
