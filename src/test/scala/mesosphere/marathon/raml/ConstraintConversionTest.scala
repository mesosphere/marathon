...
class ConstraintConversionTest extends UnitTest {
  def checkConstraintConversion(constraints: Constraint*): Unit = {
    "all constraint operators are checked" in {
      constraints
        .map(_.operator)
        .toSet shouldBe ConstraintOperator.all.toSet withClue ("We need to ensure that a case for every constraint operator is checked")
    }
