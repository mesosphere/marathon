...
class GroupApiServiceTest extends UnitTest with GroupCreation {
  ...
  "revert a version if version is provided" in {
    ...
    val updatedGroup = f.groupApiService.getUpdatedGroup(createRootGroup(), PathId.empty, GroupUpdate(version = Some(version.toOffsetDateTime)), version).futureValue
  ...
  "reverting to non-existing version throws exception" in {
    ...
    val ex = f.groupApiService.getUpdatedGroup(createRootGroup(), PathId.empty, GroupUpdate(version = Some(version.toOffsetDateTime)), version).failed.futureValue
  ...
  "scale when scaleBy provided" in {
    ...
    val updatedGroup = f.groupApiService.getUpdatedGroup(rootGroup, PathId.empty, GroupUpdate(scaleBy = Some(2)), Timestamp.now()).futureValue
  ...
  "update the group if version as well as scaleBy not provided" in {
    ...
    val updatedGroup = f.groupApiService.getUpdatedGroup(createRootGroup(), PathId.empty, GroupUpdate(apps = Some(Set(App("/app", networks = Seq(Network(mode = NetworkMode.ContainerBridge)))))), newVersion).futureValue
