...
class AppDefinitionTest extends MarathonSpec with Matchers {
  ...
  test("Validation") {
    ...
    app = correct.copy(
      container = Some(DockerDocker(
        ...
        portMappings = Seq(
          DockerDocker.PortMapping(8080, Some(0), 0, "tcp", Some("foo")),
          DockerDocker.PortMapping(8081, Some(0), 0, "tcp", Some("bar"))
        )
      )),
      portDefinitions = Nil)
    shouldNotViolate(
    ...
    app = correct.copy(
      container = Some(DockerDocker(
        network = Some(mesos.ContainerInfo.DockerInfo.Network.BRIDGE),
        portMappings = Seq(
          DockerDocker.PortMapping(8080, Some(0), 0, "tcp"),
          DockerDocker.PortMapping(8081, Some(0), 0, "tcp")
        )
      )),
      portDefinitions = Nil,
  ...
  test("SerializationRoundtrip with complex example") {
    ...
    val app3 = AppDefinition(
      ...
      container = Some(DockerDocker(image = "group/image")),
  ...
  test("Reading AppDefinition adds portIndex if you have at least one portMapping") {
    ...
    val app = AppDefinition(
      ...
      portDefinitions = Seq.empty,
      container = Some(DockerDocker(
        portMappings = Seq(DockerDocker.PortMapping())
      )),
      healthChecks = Set(HealthCheck())
    )
