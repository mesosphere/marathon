  ...
  "Apps Resource" should {
    ...
    "Create a new app with w/ Mesos containerizer and a Docker config.json" in new Fixture(
      ...
      val container = RamlContainer(
        ...
        docker = Option(DockerContainer(image = "private/image", pullConfig = Option(DockerPullConfig("pullConfigSecret"))))
      )
      val app =
        App(id = "/app", cmd = Some("cmd"), container = Option(container), secrets = Map("pullConfigSecret" -> SecretDef("/config")))
    ...
    "Create a new app with IP/CT on virtual network foo, then update it to nothing" in new FixtureWithRealGroupManager(
      initialRoot = createRootGroup(apps =
        Map(
      ...
      val app = App(
        ...
        container = Some(
          raml.Container(
            ...
            docker = Some(
              DockerContainer(
                portMappings = Option(Seq(ContainerPortMapping(containerPort = 0))),
                image = "jdef/helpme",
    ...
    "The secrets feature is NOT enabled and create app (that uses file base secrets) fails" in new Fixture(configArgs = Seq()) {
      ...
      val app = App(id = "/app", cmd = Some("cmd"), container = Option(containers), secrets = Map("bar" -> SecretDef("foo")))
      val (body, _) = prepareApp(app, groupManager, enabledFeatures = Set("secrets"))
    ...
    "Replacing an existing docker application, upgrading from host to user networking" in new Fixture {
      ...
      val app = AppDefinition(id = PathId("/app"), container = Some(Container.Docker(image = "foo")), portDefinitions = PortDefinitions(0))
    ...
    "access without authorization is denied" in new FixtureWithRealGroupManager(
      initialRoot = createRootGroup(apps = Map("/a".toRootPath -> AppDefinition("/a".toRootPath, cmd = Some("sleep"))))
    ) {
    ...
    "Allow creating app with network name with underscore" in new Fixture {
      ...
      val container = RamlContainer(`type` = EngineType.Mesos, docker = Option(DockerContainer(image = "image")))
      val app = App(
        id = "/app",
        cmd = Some("cmd"),
