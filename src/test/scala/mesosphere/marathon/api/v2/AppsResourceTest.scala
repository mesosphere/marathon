    ...
    "Replacing an existing docker application, upgrading from host to user networking" in new Fixture {
      ...
      val body =
        """{
          ...
          |}""".stripMargin.getBytes("UTF-8")
      val appUpdate = appsResource.canonicalAppUpdateFromJson(app.id, body, partialUpdate = false, newApp = false)
      ...
      val partUpdate = appsResource.canonicalAppUpdateFromJson(app.id, body, partialUpdate = true, newApp = false)
