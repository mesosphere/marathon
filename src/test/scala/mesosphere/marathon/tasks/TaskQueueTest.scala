...
class TaskQueueTest extends MarathonSpec {
  ...
  test("Filter") {
    ...
    queue.filter { case QueuedTask(app, _) => app.id == app2.id }
  ...
  test("FilterNot") {
    ...
    assert(queue.list.size == 3, "Queue should contain 3 elements.")
    queue.filterNot { case QueuedTask(app, _) => app.id == app2.id }
    assert(queue.list.size == 2, "Queue should contain 2 elements.")
  }
