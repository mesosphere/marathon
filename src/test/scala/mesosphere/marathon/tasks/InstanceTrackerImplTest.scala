...
class InstanceTrackerImplTest extends AkkaUnitTest {
  ...
  "InstanceTrackerImpl" should {
    "SerializeAndDeserialize" in new Fixture {
      val originalInstance: Instance = Instance.scheduled(AppDefinition(TEST_APP_NAME, role = Some("someRole")))
    ...
    def testList(call: InstanceTracker => InstanceTracker.InstancesBySpec)(implicit instanceTracker: InstanceTracker): Unit = {
      val instance1 = Instance.scheduled(AppDefinition(TEST_APP_NAME / "a", role = Some("someRole")))
      instanceTracker.process(Schedule(instance1)).futureValue
      val instance2 = Instance.scheduled(AppDefinition(TEST_APP_NAME / "b", role = Some("someRole")))
      instanceTracker.process(Schedule(instance2)).futureValue
      val instance3 = Instance.scheduled(AppDefinition(TEST_APP_NAME / "b", role = Some("someRole")))
    ...
    def testGetTasks(call: InstanceTracker => Seq[Instance])(implicit instanceTracker: InstanceTracker): Unit = {
      val instance1 = Instance.scheduled(AppDefinition(TEST_APP_NAME, role = Some("someRole")))
      instanceTracker.process(Schedule(instance1)).futureValue
      val instance2 = Instance.scheduled(AppDefinition(TEST_APP_NAME, role = Some("someRole")))
      instanceTracker.process(Schedule(instance2)).futureValue
      val instance3 = Instance.scheduled(AppDefinition(TEST_APP_NAME, role = Some("someRole")))
      instanceTracker.process(Schedule(instance3)).futureValue
    ...
    def testContains(count: (InstanceTracker, PathId) => Boolean)(implicit instanceTracker: InstanceTracker): Unit = {
      val task1 = Instance.scheduled(AppDefinition(TEST_APP_NAME / "a", role = Some("someRole")))
