...
class InstanceOpFactoryImplTest extends UnitTest with Inside {
  ...
  "InstanceOpFactoryImpl" should {
    "Copy SlaveID from Offer to Task" in {
      ...
      val request = InstanceOpFactory.Request(app, offer, runningInstances, additionalLaunches = 1, localRegion = None)
    ...
    "Normal app -> None (insufficient offer)" in {
      ...
      val request = InstanceOpFactory.Request(app, offer, Map.empty, additionalLaunches = 1, localRegion = None)
      val matchResult = f.instanceOpFactory.matchOfferRequest(request)
    ...
    "Resident app -> None (enough launched tasks)" in {
      ...
      When("We infer the taskOp")
      val request = InstanceOpFactory.Request(app, offer, Instance.instancesById(runningInstances), additionalLaunches = 1, localRegion = None)
      val matchResult = f.instanceOpFactory.matchOfferRequest(request)
    ...
    "update the agentInfo based on the used offer" in {
      ...
      val request = InstanceOpFactory.Request(app, offer, Map(existingReservedInstance.instanceId -> existingReservedInstance), additionalLaunches = 1, localRegion = None)
