...
class TaskIdTest extends UnitTest with Inside {
  "TaskIds" should {
    ...
    "TaskIds for resident tasks can be created from legacy taskIds" in {
      ...
      val newTaskId = Task.Id.withIncarnationCount(originalId)
    ...
    "TaskId.reservationId removes attempt from app task id" in {
      ...
      val residentTaskId = Task.Id.withIncarnationCount(originalId)
      ...
      val anotherResidentTaskId = Task.Id.withIncarnationCount(residentTaskId)
