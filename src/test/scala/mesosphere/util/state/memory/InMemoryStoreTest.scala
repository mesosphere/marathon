...
class InMemoryStoreTest extends PersistentStoreTest with Matchers with FutureTestSupport {
  ...
  test("Update an entity will increment the version") {
    ...
    val update = persistentStore.update(read.get.withNewContent("Hello again".getBytes.toIndexedSeq))
      .futureValue.asInstanceOf[InMemoryEntity]
