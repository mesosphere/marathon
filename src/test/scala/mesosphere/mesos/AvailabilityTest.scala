  ...
  "Availability" should {
    "drop offer from nodes in maintenance" in {
      ...
      Availability.offerAvailable(
        makeBasicOfferWithUnavailability(now - Duration(1, HOURS), Duration(1, DAYS)).build(),
    ...
    "accept offers from nodes not in maintenance" in {
      ...
      Availability.offerAvailable(
        makeBasicOfferWithUnavailability(now - Duration(1, DAYS), Duration(1, HOURS)).build(),
        FiniteDuration(0, SECONDS)
      ) shouldBe true
    }
    "drop offers {drainingTime} seconds before node maintenance starts" in {
      Availability.offerAvailable(
        makeBasicOfferWithUnavailability(now + Duration(200, SECONDS), Duration(1, DAYS)).build(),
        Duration(300, SECONDS)
      ) shouldBe false
    }
    "drop offer when maintenance with infinite duration" in {
      Availability.offerAvailable(
        makeBasicOfferWithUnavailability(now - Duration(1, HOURS)).build(),
        FiniteDuration(0, SECONDS)
      ) shouldBe false
    }
  }
