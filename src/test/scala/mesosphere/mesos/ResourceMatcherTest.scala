...
class ResourceMatcherTest extends UnitTest with Inside {
  ...
  "ResourceMatcher" should {
    "match with app.disk == 0, even if no disk resource is contained in the offer" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(offer, app, knownInstances = Seq.empty, unreservedResourceSelector, config, homeFaultDomain = None)
    ...
    "match resources success with preserved reservations" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), ResourceSelector.reservedWithLabels(Set(ResourceRole.Unreserved, "marathon"), labels), config, homeFaultDomain = None)
      ...
      ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), ResourceSelector.any(Set(ResourceRole.Unreserved, "marathon")), config, homeFaultDomain = None) shouldBe a[ResourceMatchResponse.NoMatch]
    }
    ...
    "dynamically reserved resources are matched if they have no labels" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), ResourceSelector.any(Set(ResourceRole.Unreserved, "marathon")), config, homeFaultDomain = None)
    ...
    "ResourceSelector.reservedWithLabels should not match disk resource without label" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), ResourceSelector.reservedWithLabels(Set(ResourceRole.Unreserved, "marathon"), Map("some" -> "label")), config, homeFaultDomain = None
      )
    ...
    "match resources success with preserved roles" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), ResourceSelector.any(Set("marathon")), config, homeFaultDomain = None)
    ...
    "match resources failure because of incorrect roles" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), unreservedResourceSelector, config, homeFaultDomain = None)
    ...
    "resource matcher should not respond with NoOfferMatchReason.UnfulfilledRole if role matches" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(offer, app, knownInstances = Seq.empty, ResourceSelector.any(Set("A", "B")), config, homeFaultDomain = None)
    ...
    "match resources success with constraints and old tasks in previous version" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(offer, app, instances, unreservedResourceSelector, config, homeFaultDomain = None)
    ...
    "match disk won't allocate resources across disk different paths" in {
      ...
      ResourceMatcher.matchResources(
        ...
        knownInstances = Seq(),
        ResourceSelector.reservable, config, homeFaultDomain = None) shouldBe a[ResourceMatchResponse.NoMatch]

      val resourceMatchResponse = ResourceMatcher.matchResources(
        offerSufficeWithMultOffers, app,
        knownInstances = Seq(),
        ResourceSelector.reservable, config, homeFaultDomain = None)
    ...
    "match disk enforces constraints" in {
      ...
      ResourceMatcher.matchResources(
        ...
        knownInstances = Seq(),
        ResourceSelector.reservable, config, homeFaultDomain = None) shouldBe a[ResourceMatchResponse.Match]
    }
    ...
    "a Reserved instance prevents creation of another reservation when hostname constraint is set" in {
      ...
      val response = ResourceMatcher.matchResources(
        offer, app, knownInstances = Seq(instance), ResourceSelector.reservable, config, homeFaultDomain = None)
    ...
    "match offers with maintenance mode and enabled feature should match" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(offer, app, knownInstances = Seq.empty, unreservedResourceSelector, maintenanceEnabledConf, homeFaultDomain = None)
