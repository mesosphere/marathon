...
class ResourceMatcherTest extends UnitTest with Inside {
  ...
  "ResourceMatcher" should {
    "match with app.disk == 0, even if no disk resource is contained in the offer" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(offer, app, runningInstances = Seq.empty, unreservedResourceSelector, FiniteDuration(config.drainingTime(), SECONDS))
    ...
    "match resources success" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(offer, app, runningInstances = Seq.empty, unreservedResourceSelector, drainingTime)
    ...
    "match resources success with preserved reservations" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        runningInstances = Seq(), ResourceSelector.reservedWithLabels(Set(ResourceRole.Unreserved, "marathon"), labels), drainingTime)
      ...
      ResourceMatcher.matchResources(
        offer, app,
        runningInstances = Seq(), ResourceSelector.any(Set(ResourceRole.Unreserved, "marathon")), drainingTime) shouldBe a[ResourceMatchResponse.NoMatch]
    }
    ...
    "dynamically reserved resources are matched if they have no labels" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        runningInstances = Seq(), ResourceSelector.any(Set(ResourceRole.Unreserved, "marathon")), drainingTime)
    ...
    "ResourceSelector.reservedWithLabels should not match disk resource without label" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        runningInstances = Seq(), ResourceSelector.reservedWithLabels(Set(ResourceRole.Unreserved, "marathon"), Map("some" -> "label")), drainingTime
      )
    ...
    "match resources success with preserved roles" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        runningInstances = Seq(), ResourceSelector.any(Set("marathon")), drainingTime)
    ...
    "match resources failure because of incorrect roles" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        runningInstances = Seq(), unreservedResourceSelector, drainingTime)
    ...
    "resource matcher should not respond with NoOfferMatchReason.UnfulfilledRole if role matches" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(offer, app, runningInstances = Seq.empty, ResourceSelector.any(Set("A", "B")), drainingTime)
