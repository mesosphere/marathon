  ...
  "ResourceMatcher" should {
    ...
    "match resources success with preserved reservations" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), ResourceSelector.reservedWithLabels(Set(ResourceRole.Unreserved, "marathon"), labels), config, localRegion = None)
      ...
      ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), ResourceSelector.any(Set(ResourceRole.Unreserved, "marathon")), config, localRegion = None) shouldBe a[ResourceMatchResponse.NoMatch]
    }
    ...
    "dynamically reserved resources are matched if they have no labels" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), ResourceSelector.any(Set(ResourceRole.Unreserved, "marathon")), config, localRegion = None)
    ...
    "ResourceSelector.reservedWithLabels should not match disk resource without label" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), ResourceSelector.reservedWithLabels(Set(ResourceRole.Unreserved, "marathon"), Map("some" -> "label")), config, localRegion = None
      )
    ...
    "match resources success with preserved roles" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), ResourceSelector.any(Set("marathon")), config, localRegion = None)
    ...
    "match resources failure because of incorrect roles" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(
        offer, app,
        knownInstances = Seq(), unreservedResourceSelector, config, localRegion = None)
    ...
    "resource matcher should not respond with NoOfferMatchReason.UnfulfilledRole if role matches" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(offer, app, knownInstances = Seq.empty, ResourceSelector.any(Set("A", "B")), config, localRegion = None)
    ...
    "match resources success with constraints and old tasks in previous version" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(offer, app, instances, unreservedResourceSelector, config, localRegion = None)
    ...
    "match disk won't allocate resources across disk different paths" in {
      ...
      ResourceMatcher.matchResources(
        ...
        knownInstances = Seq(),
        ResourceSelector.reservable, config, localRegion = None) shouldBe a[ResourceMatchResponse.NoMatch]

      val resourceMatchResponse = ResourceMatcher.matchResources(
        offerSufficeWithMultOffers, app,
        knownInstances = Seq(),
        ResourceSelector.reservable, config, localRegion = None)
    ...
    "match disk enforces constraints" in {
      ...
      ResourceMatcher.matchResources(
        ...
        knownInstances = Seq(),
        ResourceSelector.reservable, config, localRegion = None) shouldBe a[ResourceMatchResponse.Match]
    }
    ...
    "a Reserved instance prevents creation of another reservation when hostname constraint is set" in {
      ...
      val response = ResourceMatcher.matchResources(
        offer, app, knownInstances = Seq(instance), ResourceSelector.reservable, config, localRegion = None)
    ...
    "match offers with maintenance mode and enabled feature should match" in {
      ...
      val resourceMatchResponse = ResourceMatcher.matchResources(offer, app, knownInstances = Seq.empty, unreservedResourceSelector, maintenanceEnabledConf, localRegion = None)
