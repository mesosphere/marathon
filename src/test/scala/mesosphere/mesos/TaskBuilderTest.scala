...
class TaskBuilderTest extends MarathonSpec with Matchers {
  ...
  test("UniqueHostNameAndClusterAttribute") {
    val app = MarathonTestHelper.makeBasicApp().copy(
      instances = 10,
      constraints = Set(
        Protos.Constraint.newBuilder.setField("spark")
          .setOperator(Protos.Constraint.Operator.CLUSTER).setValue("enabled").build,
        Protos.Constraint.newBuilder.setField("hostname").setOperator(Protos.Constraint.Operator.UNIQUE).build
      )
    )
  ...
  test("PortsEnv") {
    val env = TaskBuilder.portsEnv(Seq(0, 0), Seq(1001, 1002), Seq(None, None))
  ...
  test("PortsNamedEnv") {
    val env = TaskBuilder.portsEnv(Seq(0, 0), Seq(1001, 1002), Seq(Some("http"), Some("https")))
    assert("1001" == env("PORT"))
    assert("1001" == env("PORT0"))
    assert("1002" == env("PORT1"))
  ...
  test("DeclaredPortsEnv") {
    val env = TaskBuilder.portsEnv(Seq(80, 8080), Seq(1001, 1002), Seq(None, None))
    assert("1001" == env("PORT"))
    assert("1001" == env("PORT0"))
    assert("1002" == env("PORT1"))
  ...
  test("DeclaredPortsEnvNamed") {
    val env = TaskBuilder.portsEnv(Seq(80, 8080, 443), Seq(1001, 1002, 1003), Seq(Some("http"), None, Some("https")))
    assert("1001" == env("PORT"))
    assert("1001" == env("PORT0"))
    assert("1002" == env("PORT1"))
  ...
  test("taskKillGracePeriod specified in app definition is passed through to TaskInfo") {
    ...
    val builder = new TaskBuilder(app, s => Task.Id(s.toString), MarathonTestHelper.defaultConfig(), None)
