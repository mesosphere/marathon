...
class TaskBuilderTest extends UnitTest {
  ...
  "TaskBuilder" should {
    ...
    "BuildIfMatchesWithExecutorResources" in {
      ...
      assert(taskInfo.hasExecutor)
      assert(
        taskInfo.getExecutor.getResourcesList == Seq(
    ...
    "BuildIfMatchesWithRackIdConstraint" in {
      ...
      val resourceMatch = RunSpecOfferMatcher.matchOffer(app, offer, s, config.defaultAcceptedResourceRolesSet, config, Seq.empty)
    ...
    "TaskContextEnv will provide label env safety" in {
      ...
      val env4 = TaskBuilder
        .taskContextEnv(runSpec = runSpec, Some(Task.LegacyId(runSpec.id, ".", uuid)))
    ...
    "taskKillGracePeriod specified in app definition is passed through to TaskInfo" in {
      ...
      val resourceMatch =
        RunSpecOfferMatcher.matchOffer(app, offer, runningInstances.toIndexedSeq, config.defaultAcceptedResourceRolesSet, config, Seq.empty)
      assert(resourceMatch.isInstanceOf[ResourceMatchResponse.Match])
      val matches = resourceMatch.asInstanceOf[ResourceMatchResponse.Match]
  ...
  def buildIfMatches(
    ...
    val builder = new TaskBuilder(
      app,
      taskId,
      MarathonTestHelper.defaultConfig(mesosRole = mesosRole, acceptedResourceRoles = acceptedResourceRoles, envVarsPrefix = envVarsPrefix)
    )
    ...
    val resourceMatch = RunSpecOfferMatcher.matchOffer(
      ...
      offer,
      Seq.empty,
      acceptedResourceRoles.getOrElse(config.defaultAcceptedResourceRolesSet),
      config,
      Seq.empty
    )
