...
trait PodsValidation extends GeneralPurposeCombinators {
  ...
  def podValidator(
      config: MarathonConf,
      mesosMasterVersion: Option[SemanticVersion] = Some(SemanticVersion.zero),
      roleSettings: RoleSettings
  ): Validator[Pod] =
    podValidator(
      config.availableFeatures,
      mesosMasterVersion.getOrElse(SemanticVersion.zero),
  ...
  def podValidator(
      ...
      mesosMasterVersion: SemanticVersion,
