...
trait AppValidation {
  ...
  def validateCanonicalAppAPI(enabledFeatures: Set[String], defaultNetworkName: () => Option[String]): Validator[App] =
    forAll(
  ...
  private def validBasicAppDefinition(enabledFeatures: Set[String]): Validator[App] =
    validator[App] { app =>
    ...
    isTrue("AppDefinition must either contain one of 'cmd' or 'args', and/or a 'container'.") { app =>
      ...
      val container = app.container.exists { ct =>
        (ct.docker, ct.appc, ct.`type`) match {
          case (Some(_), None, EngineType.Docker) | (Some(_), None, EngineType.Mesos) | (None, Some(_), EngineType.Mesos) => true
          case _ => false
        }
      }
