...
class PodsResource @Inject() (val config: MarathonConf)(implicit
  ...
  private implicit val normalizer = PodNormalization.apply(PodNormalization.Configuration(config.defaultNetworkName.toOption))
