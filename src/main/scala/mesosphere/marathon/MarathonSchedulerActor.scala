...
class MarathonSchedulerActor private (
  ...
  /**
    ...
    */
  val lockedRunSpecs = collection.mutable.Map.empty[PathId, Long]
  ...
  /**
    ...
    */
  def withLockFor[A](runSpecIds: Set[PathId])(f: Long => A): Option[A] = {
  ...
  def removeLocks(runSpecIds: Iterable[PathId], lockVersion: Long): Unit =
    runSpecIds.foreach { runSpecId => removeLock(runSpecId, lockVersion) }
  def removeLock(runSpecId: PathId, lockVersion: Long): Unit = {
  ...
  def addLocks(runSpecIds: Set[PathId]): Long = {
    val lockVersion = getNextLockVersion()
    runSpecIds.foreach { id => addLock(id, lockVersion) }
    lockVersion
  }

  def addLock(runSpecId: PathId, lockVersion: Long): Unit = {
...
object MarathonSchedulerActor {
  ...
  case class RemoveLocks(runSpecId: Iterable[PathId], lockVersion: Long)
...
class SchedulerActions(
  ...
  def reconcileTasks(driver: SchedulerDriver): Future[Status] =
    async {
      ...
      (instances.allSpecIdsWithInstances -- runSpecIds).foreach { unknownId =>
        ...
        logger.warn(
          s"Instances reference runSpec $unknownId, which does not exist. Will now decommission. [${orphanedInstances.map(_.instanceId)}]."
        )
  ...
  def scale(runSpec: RunSpec): Future[Done] =
    async {
      ...
      val ScalingProposition(instancesToDecommission, instancesToStart) =
        ScalingProposition.propose(instances, Seq.empty, killToMeetConstraints, targetCount, runSpec.killSelection, runSpec.id)
  ...
  def scale(runSpecId: PathId): Future[Done] =
    async {
