...
/**
  ...
  */
private[upgrade] object DeploymentPlanReverter extends DeploymentPlanGroupChangesReverter {
...
sealed trait DeploymentPlanGroupChangesReverter {
  private[this] val log = LoggerFactory.getLogger(getClass)
  ...
  /**
    ...
    */
  def revertGroupChanges(version: Timestamp, groupChanges: Seq[(Option[Group], Option[Group])])(group: Group): Group = {
    // sort groups so that inner groups are processed first
    val sortedGroupChanges = sortGroupChangesByPathId(groupChanges)
      ...
      case (result, groupUpdate) =>
        groupUpdate match {
          ...
          case (None, Some(newGroup)) =>
            revertGroupAddition(result, newGroup, version)
  ...
  private[this] def sortGroupChangesByPathId(groupChanges: Seq[(Option[Group], Option[Group])]) = {
    groupChanges.sortWith {
  ...
  private[this] def revertGroupAddition(result: Group, newGroup: Group, version: Timestamp): Group = {
  ...
  private[this] def revertDependencyChanges(oldGroup: Group, newGroup: Group)(group: Group): Group = {
  ...
  private[this] def revertGroupRemoval(oldGroup: Group)(existingGroup: Group): Group = {
