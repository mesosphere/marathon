...
trait DefaultConversions {
  ...
  implicit def optionConversion[A, B](implicit writer: Writes[A, B]): Writes[Option[A], Option[B]] =
    Writes { option =>
      option.map(writer.write)
    }

  implicit def seqConversion[A, B](implicit writer: Writes[A, B]): Writes[Seq[A], Seq[B]] =
    Writes { seq =>
      seq.map(writer.write)
    }

  implicit def javaListToSeqConversion[A, B](implicit writer: Writes[A, B]): Writes[java.util.List[A], Seq[B]] =
    Writes { list =>
      list.iterator.asScala.map(writer.write).toSeq
    }

  implicit def setConversion[A, B](implicit writer: Writes[A, B]): Writes[Set[A], Set[B]] =
    Writes { set =>
      set.map(writer.write)
    }

  implicit def mapConversion[K1, V1, K2, V2](implicit key: Writes[K1, K2], value: Writes[V1, V2]): Writes[Map[K1, V1], Map[K2, V2]] =
    Writes { map =>
      map.map {
