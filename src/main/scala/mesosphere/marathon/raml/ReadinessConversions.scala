...
trait ReadinessConversions {
  ...
  implicit val readinessCheckResultWrites: Writes[core.readiness.ReadinessCheckResult, raml.TaskReadinessCheckResult] = Writes { result =>
    raml.TaskReadinessCheckResult(
      ...
      ready = result.ready,
      lastResponse =
        result.lastResponse.map(response => raml.ReadinessCheckHttpResponse(response.status, response.contentType, response.body))
    )
  }
  ...
  implicit val appReadinessProtoRamlWriter: Writes[Protos.ReadinessCheckDefinition, ReadinessCheck] = Writes { rc =>
    ReadinessCheck(
      ...
      httpStatusCodesForReady =
        if (rc.getHttpStatusCodeForReadyCount > 0) Option(rc.getHttpStatusCodeForReadyList.asScala.iterator.map(_.intValue()).toSet)
        else None,
