...
trait ContainerConversion extends HealthCheckConversion with VolumeConversion with NetworkConversion {
  ...
  implicit val containerWrites: Writes[state.Container, Container] = Writes { container =>
    ...
    def create(
        kind: EngineType,
        docker: Option[DockerContainer] = None,
        appc: Option[AppCContainer] = None,
      ...
      Container(
        ...
        docker = docker,
        appc = appc,
        volumes = container.volumes.toRaml,
    ...
    container match {
      ...
      case mesos: state.Container.MesosDocker =>
        create(EngineType.Mesos, docker = Some(mesos.toRaml[DockerContainer]), linuxInfo = container.linuxInfo.toRaml)
      case mesos: state.Container.MesosAppC =>
        create(EngineType.Mesos, appc = Some(mesos.toRaml[AppCContainer]), linuxInfo = None) // Linux == None as it is deprecated
  ...
  implicit val appcProtoToRamlWriter: Writes[Protos.ExtendedContainerInfo.MesosAppCInfo, AppCContainer] = Writes { appc =>
    AppCContainer(
      ...
      labels =
        appc.whenOrElse(_.getLabelsCount > 0, _.getLabelsList.flatMap(_.fromProto)(collection.breakOut), AppCContainer.DefaultLabels),
  ...
  implicit val dockerProtoToRamlWriter: Writes[Protos.ExtendedContainerInfo.DockerInfo, DockerContainer] = Writes { docker =>
    DockerContainer(
      ...
      parameters = docker
        .whenOrElse(_.getParametersCount > 0, _.getParametersList.map(_.toRaml)(collection.breakOut), DockerContainer.DefaultParameters),
      portMappings = Option
        .empty[Seq[ContainerPortMapping]]
        .unless(
          docker.when(
            _.getOBSOLETENetwork != Mesos.ContainerInfo.DockerInfo.Network.HOST,
            _.getOBSOLETEPortMappingsList.map(_.toRaml)(collection.breakOut)
          )
        ),
  ...
  implicit val dockerPullConfigProtoRamlWriter: Writes[Protos.ExtendedContainerInfo.DockerInfo.ImagePullConfig, DockerPullConfig] = Writes {
    pullConfig =>
      pullConfig
        .when(
          _.getType == Protos.ExtendedContainerInfo.DockerInfo.ImagePullConfig.Type.SECRET,
          _ => {
