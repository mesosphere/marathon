...
case class Group(
  ...
  /**
    * Compute the hashCode of a groups only by id.
    * Override the default hashCode implementation generated by scalac, which is very expensive.
    *
    * Please note: scalac will generate an equals method for this case class, so it is not provided explicitly.
    */
