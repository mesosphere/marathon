...
trait ZookeeperConf extends ScallopConf {
  ...
  val zooKeeperHostString = opt[String]("zk_hosts",
    descr = "[DEPRECATED use zk_marathon] The list of ZooKeeper servers for storing state",
    default = Some("localhost:2181"))

  @Deprecated
  val zooKeeperPath = opt[String]("zk_state",
    descr = "[DEPRECATED use zk_marathon] Path in ZooKeeper for storing state",
  ...
  val zooKeeperUrl = opt[String]("zk_marathon",
