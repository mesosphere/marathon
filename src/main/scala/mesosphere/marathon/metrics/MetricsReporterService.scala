...
class MetricsReporterService @Inject() (config: MetricsConf, registry: MetricRegistry) extends AbstractIdleService {
  ...
  private[this] def startDatadog(dataDog: String): DatadogReporter = {
    val url = new URL(dataDog)
    val params = Option(url.getQuery).getOrElse("").split("&").collect { case QueryParam(k, v) => k -> v }.toMap

    val transport = url.getProtocol match {
      case "http" =>
