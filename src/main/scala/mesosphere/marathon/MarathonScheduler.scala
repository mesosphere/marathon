...
class MarathonScheduler(
  ...
  @volatile private[this] var localRegion: Option[Region] = Option.empty
  ...
  protected def updateLocalRegion(masterInfo: MasterInfo): Unit = {
    if (masterInfo.hasDomain && masterInfo.getDomain.hasFaultDomain) {
      localRegion = Some(Region(
  ...
  /**
    ...
    */
  def getLocalRegion: Option[Region] = localRegion
