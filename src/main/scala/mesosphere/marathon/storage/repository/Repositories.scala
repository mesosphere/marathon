...
class AppRepositoryImpl[K, C, S](persistenceStore: PersistenceStore[K, C, S])(implicit
    ...
    ctx: ExecutionContext
) extends PersistenceStoreVersionedRepository[PathId, AppDefinition, K, C, S](persistenceStore, _.id, _.version.toOffsetDateTime)
    with AppRepository {
...
) extends PersistenceStoreVersionedRepository[PathId, TaskFailure, K, C, S](persistenceStore, _.appId, _.version.toOffsetDateTime)
