...
class MigrationTo18100(instanceRepository: InstanceRepository, persistenceStore: PersistenceStore[_, _, _])
...
object MigrationTo18100 extends StrictLogging {
  ...
  val migrationFlow = Flow[JsValue].filter { jsValue =>
    (jsValue \ "state" \ "condition" \ "str")
      .orElse(jsValue \ "state" \ "condition")
      .validate[String]
      .map { condition =>
