...
private[leadership] object AbdicateOnConnectionLossActor {
  def props(zk: ZooKeeperClient, electionService: ElectionService): Props = {
    Props(new AbdicateOnConnectionLossActor(zk, electionService))
  }
...
private[impl] class AbdicateOnConnectionLossActor(zk: ZooKeeperClient,
                                                  electionService: ElectionService) extends Actor with ActorLogging {
