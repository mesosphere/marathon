...
class OfferOperationFactory(metrics: Metrics, private val principalOpt: Option[String], private val roleOpt: Option[String]) {
  ...
  private[this] lazy val role: String = roleOpt match {
    ...
    case _ => throw WrongConfigurationException("No role set. Set --mesos_role to enable using local volumes in Marathon.")
  }
