  ...
  private[this] def updatedHealthOrState(
    ...
    maybeCurrent match {
      case Some(current) =>
        ...
        val checked = update.hasCheckStatus && (!current.hasCheckStatus || current.getCheckStatus != update.getCheckStatus)
        val changed = healthy || checked || current.getState != update.getState
...
object Task {
  ...
  object Id {
    ...
    /**
      ...
      * task container's name. It includes the first incarnation.
      *
