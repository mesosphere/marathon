...
trait TaskKillService {
  /**
    ...
    */
  def killTasks(tasks: Iterable[Task], reason: TaskKillReason): Future[Unit]
  ...
  /**
    ...
    * @return a future that is completed when all tasks are killed.
    */
  def killTask(task: Task, reason: TaskKillReason): Future[Unit]
  ...
  /**
    ...
    * @return a future that is completed when all tasks are killed.
    */
  def killUnknownTask(taskId: Task.Id, reason: TaskKillReason): Future[Unit]
}
