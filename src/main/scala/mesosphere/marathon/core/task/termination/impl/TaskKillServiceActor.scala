...
private[impl] class TaskKillServiceActor(
  ...
  def killUnknownTaskById(taskId: Task.Id, promise: Promise[Unit]): Unit = {
  ...
  def setupProgressActor(taskIds: Iterable[Task.Id], promise: Promise[Unit]): Unit = {
...
private[termination] object TaskKillServiceActor {
  ...
  case class KillUnknownTaskById(taskId: Task.Id, promise: Promise[Unit]) extends Request
