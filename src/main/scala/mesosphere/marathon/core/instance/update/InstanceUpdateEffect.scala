...
object InstanceUpdateEffect {
  ...
  case class Update(instance: Instance, oldState: Option[Instance], events: Seq[MarathonEvent]) extends InstanceUpdateEffect

  /** The instance with the given Id must be expunged */
  case class Expunge(instance: Instance, events: Seq[MarathonEvent]) extends InstanceUpdateEffect
