...
object ExternalVolumes {
  ...
  def validExternalVolume: Validator[ExternalVolume] =
    (ev: ExternalVolume) => providers.get(ev.external.provider) match {
  ...
  def validRamlVolume(container: raml.Container): Validator[AppExternalVolume] =
    (ev: AppExternalVolume) => ExternalVolumeRamlHelpers.getProvider(ev.external).flatMap(providers.get(_)) match {
      case Some(p) =>
  ...
  def validRootGroup(): Validator[RootGroup] =
    (rootGroup: RootGroup) => providers.values.map { provider =>
