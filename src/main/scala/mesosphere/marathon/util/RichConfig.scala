...
class RichConfig(val config: Config) extends AnyVal {
  ...
  private def list[A, B](path: String, nonEmpty: Config => util.List[A])(implicit toScala: A => B): Seq[B] = {
    if (config.hasPath(path)) {
  ...
  def boolList(path: String): Seq[Boolean] = list(path, _.getBooleanList(path))
  ...
  def bytesList(path: String): Seq[Long] = list(path, _.getBytesList(path))
  ...
  def doubleList(path: String): Seq[Double] = list(path, _.getDoubleList(path))
  ...
  def durationList(path: String): Seq[Duration] = list(path, _.getDurationList(path))
  ...
  def intList(path: String): Seq[Int] = list(path, _.getIntList(path))
  ...
  def longList(path: String): Seq[Long] = list(path, _.getLongList(path))
  ...
  def memorySizeList(path: String): Seq[ConfigMemorySize] = list(path, _.getMemorySizeList(path))
  ...
  def numberList(path: String): Seq[Number] = list(path, _.getNumberList(path))
  ...
  def stringList(path: String): Seq[String] = list(path, _.getStringList(path))
}
