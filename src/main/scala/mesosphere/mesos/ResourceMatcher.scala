...
import mesosphere.marathon.state.{ AppDefinition, ResourceRole, PersistentVolume, DiskType }
...
object ResourceMatcher {
  ...
  def matchResources(offer: Offer, app: AppDefinition, runningTasks: => Iterable[Task],
    ...
    val diskMatch = if (needToReserveDisk)
      diskResourceMatch(
        app.disk,
        app.persistentVolumes,
        ScalarMatchResult.Scope.IncludingLocalVolumes)
    else
      diskResourceMatch(app.disk, Nil, ScalarMatchResult.Scope.ExcludingLocalVolumes)
  ...
  private[this] def matchDiskResource(
    ...
    def findMatches(
      ...
      localVolumes: List[(Option[Source], PersistentVolume)] = Nil): Either[DiskResourceNoMatch, DiskResourceMatch] = {
