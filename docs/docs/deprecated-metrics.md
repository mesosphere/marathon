...
* statsd via `--reporter_datadog` (datadog reporter supports statsd).
...
`service.mesosphere.marathon.app.count` (gauge) - The number of defined apps. Be advised that high app number may lead to a degraded performance.
