...
case "$MODE" in
  build)
    if [ -z $MARATHON_DOCS_VERSION ]; then
      ...
      BUNDLE_GEMFILE=/jekyll/Gemfile bundle exec jekyll build --config _config.yml -d _site
    else
      echo "Generating docs for version $MARATHON_DOCS_VERSION"
      BUNDLE_GEMFILE=/jekyll/Gemfile bundle exec jekyll build --config "_config.yml,_config.$MARATHON_DOCS_VERSION.yml" -d "_site/$MARATHON_DOCS_VERSION/"
    fi
    ;;
  watch)
