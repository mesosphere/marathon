...
`localhost:8080` and you should see the Marathon UI.
    ...
    ./bin/start --master zk://zk1.foo.bar:2181,zk2.foo.bar:2181/mesos --zk_marathon zk://zk1.foo.bar:2181,zk2.foo.bar:2181/marathon

Marathon uses `--master` to find the Mesos masters, and `--zk_marathon` to find Zookeepers
...
`http://localhost:8080`, and you should see the Marathon UI.
    ...
    ./bin/start --master local --zk_marathon zk://localhost:2181/marathon
...
easy editing, set the `assets_path` flag when running Marathon:
    ...
    ./bin/start --master local --zk_marathon zk://localhost:2181/marathon --assets_path src/main/resources/assets/
...
* `MESOS_NATIVE_LIBRARY`: `bin/start` searches the common installation paths,
  ...
  For example:
      ...
      MESOS_NATIVE_LIBRARY=/Users/bob/libmesos.dylib ./bin/start --master local --zk_marathon zk://localhost:2181/marathon
