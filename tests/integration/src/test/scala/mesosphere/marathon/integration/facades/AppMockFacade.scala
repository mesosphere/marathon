...
case class AppMockFacade(task: ITEnrichedTask) extends StrictLogging {
  ...
  def suicide()(implicit ec: ExecutionContext, system: ActorSystem, mat: Materializer): Future[Done] = async {
    ...
    assert(result.status.isSuccess(), s"App suicide failed with status ${result.status} for task $task")
    Done
  }
  ...
  def get(path: String)(implicit ec: ExecutionContext, system: ActorSystem, mat: Materializer): Future[String] = async {
    ...
    assert(result.status.isSuccess(), s"App data retrieval failed with status ${result.status} for task $task")
