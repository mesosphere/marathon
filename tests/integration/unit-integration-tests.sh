...
DOCKER_OPTIONS="run --entrypoint=/bin/bash --rm --name marathon-itests-$BUILD_ID --net host --privileged -e COVERALLS_REPO_TOKEN=$COVERALL_REPO_TOKEN -e TARGETS_DIR=$TARGETS_DIR -e RUN_DOCKER_INTEGRATION_TESTS=$RUN_DOCKER_INTEGRATION_TESTS -e MARATHON_MAX_TASKS_PER_OFFER=$MARATHON_MAX_TASKS_PER_OFFER -v $WORKSPACE:/marathon -v $TARGET:/marathon/target -v /var/run/docker.sock:/var/run/docker.sock -v /etc/hosts:/etc/hosts -i"

DOCKER_CMD="/usr/local/bin/sbt -Dsbt.log.format=false coverage doc test integration:test scapegoat coverageReport coveralls mesos-simulation/integration:test"
